export const GROQ_CACHE_MODEL = process.env.GROQ_MODEL || 'openai/gpt-oss-120b';

export const AUTO_NIRMAN_CHAT_SYSTEM_PROMPT = `
...
export const AUTO_NIRMAN_MAP_SYSTEM_PROMPT = `
...
export function getGroqCachedTokens(data: unknown) {
  const usage = (data as { usage?: { prompt_tokens?: number; prompt_tokens_details?: { cached_tokens?: number } } })?.usage;
  ...
  return {
    promptTokens,
    cachedTokens,
    cacheHitRate: promptTokens > 0 ? Math.round((cachedTokens / promptTokens) * 100) : 0,
  };
}
