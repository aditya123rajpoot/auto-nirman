...
function FloatingInput({
  ...
  return (
    <div className="relative">
      {/* Glow border */}
      <div className={`absolute -inset-[1px] rounded-2xl transition-all duration-500 ${focused ? 'bg-gradient-to-r from-cyan-300/70 via-blue-400/60 to-indigo-400/70 shadow-[0_0_28px_rgba(34,211,238,0.18)]' : 'bg-white/8'}`} />
      <div className="group relative overflow-hidden rounded-2xl border border-white/10 bg-[#0b1728] shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_16px_36px_rgba(0,0,0,0.22)] transition-all duration-300 hover:border-cyan-300/25 hover:bg-[#0d1f35]">
        <div className={`pointer-events-none absolute inset-0 transition-opacity duration-500 ${focused ? 'opacity-100' : 'opacity-0'} bg-[radial-gradient(circle_at_12%_0%,rgba(34,211,238,0.16),transparent_36%)]`} />
        <input
          id={id}
          type={type}
          value={value}
        ...
        <div className={`absolute bottom-0 left-0 right-0 h-[2px] transition-all duration-500 ${focused ? 'bg-gradient-to-r from-cyan-300 via-blue-400 to-indigo-400' : 'bg-white/10'}`} />
        {rightElement && (
      ...
      <div className="w-full lg:w-[45%] flex items-center justify-center px-4 pb-8 pt-20 sm:px-6 lg:min-h-screen lg:py-12 relative">
        ...
        >
          ...
          <div className="lg:hidden flex items-center justify-between gap-3 mb-4">
            ...
            <span className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-cyan-100">Secure</span>
          </div>

          {/* Mobile tagline */}
          <div className="lg:hidden mb-5 rounded-xl border border-cyan-500/10 bg-cyan-500/5 px-3 py-2.5">
            <p className="text-xs font-semibold leading-5 text-slate-300">Sign in to continue your BOQ, estimate, map, and AI workspace.</p>
          </div>

          {/* Header */}
          <div className="mb-5 sm:mb-8">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
              className="hidden items-center gap-2 bg-cyan-500/10 border border-cyan-500/20 rounded-full px-3 py-1 mb-4 lg:inline-flex">
              <span className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-pulse" />
              <span className="text-cyan-400 text-xs font-semibold tracking-wider uppercase">Secure Login</span>
            </motion.div>
            <h2 className="text-3xl font-black text-white mb-1 tracking-tight sm:text-4xl lg:text-3xl">Welcome back</h2>
          ...
          <form onSubmit={handleLogin} className="space-y-3.5 sm:space-y-4">
            ...
            <motion.button type="submit" disabled={loading || !email || !password}
              ...
              className="relative w-full overflow-hidden rounded-2xl py-3.5 text-sm font-black uppercase tracking-[0.08em] shadow-[0_18px_46px_rgba(37,99,235,0.26)] transition-all disabled:cursor-not-allowed disabled:opacity-40 sm:py-4 group mt-2">
              <div className="absolute inset-0 bg-gradient-to-r from-cyan-300 via-blue-500 to-indigo-600 transition-all duration-300 group-hover:from-cyan-200 group-hover:via-sky-400 group-hover:to-indigo-500" />
              ...
              <div className="absolute inset-0 opacity-70 bg-[radial-gradient(ellipse_at_50%_120%,_rgba(6,182,212,0.55),_transparent_70%)]" />
          ...
          <div className="space-y-2.5 sm:space-y-3">
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
              ...
              className="w-full py-3 rounded-xl border border-white/8 bg-white/4 hover:bg-white/8 hover:border-cyan-400/30 text-white text-sm font-semibold flex items-center justify-center gap-3 transition-all duration-300 disabled:opacity-50 sm:py-3.5">
              {googleLoading
            ...
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
              ...
              className="w-full py-3 rounded-xl border border-white/5 hover:bg-purple-500/8 hover:border-purple-400/20 text-slate-600 hover:text-slate-300 text-sm font-medium flex items-center justify-center gap-3 transition-all duration-300 sm:py-3.5">
