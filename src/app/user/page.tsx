...
// ── Replace these with your real auth data ──
const mockUser = {
  name: 'Aditya Rathour',
  email: 'aditya@autonirman.com',
    ...
    <div className="relative min-h-screen pt-20 px-4 pb-12 bg-black text-white font-sans overflow-hidden">
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400 text-sm">{mockUser.company}</p>
          <span className="mt-2 flex items-center gap-1 text-xs bg-blue-400/10 border border-blue-400/30 text-blue-300 rounded-full px-3 py-1">
            <FaCrown size={10} /> {mockUser.plan} Plan
          </span>
        </div>
        ...
        {activeTab === 'profile' && (
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-6 space-y-4">
            ...
            {[
              { icon: FaUser,          label: 'Full Name',  value: mockUser.name },
              { icon: FaEnvelope,      label: 'Email',      value: mockUser.email },
              { icon: FaPhone,         label: 'Phone',      value: mockUser.phone },
              { icon: FaMapMarkerAlt,  label: 'Location',   value: mockUser.location },
              { icon: FaBuilding,      label: 'Company',    value: mockUser.company },
              { icon: FaCalendarAlt,   label: 'Member since', value: mockUser.joinDate },
            ].map((row, i) => (
            ...
            {mockProjects.map((p, i) => (
              <div key={i} className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="text-xs font-medium px-2 py-0.5 rounded-full border ml-2 shrink-0"
        ...
        {activeTab === 'plan' && (
          <div className="space-y-4">
            <div className="bg-gradient-to-br from-blue-900/40 to-purple-900/40 backdrop-blur-lg border border-blue-400/30 rounded-2xl p-6 text-center">
              ...
              <h2 className="text-2xl font-bold">{mockUser.plan} Plan</h2>
              <p className="text-slate-400 text-sm mt-1">Valid until {mockUser.planExpiry}</p>
              <button className="mt-4 bg-blue-400 text-black font-semibold rounded-xl px-6 py-2.5 text-sm hover:bg-blue-300 transition-colors">
                Upgrade Plan
              </button>
            </div>
